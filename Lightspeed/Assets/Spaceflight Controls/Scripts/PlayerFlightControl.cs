...
public class PlayerFlightControl : MonoBehaviour
{
	...
	void Start() {
        ...
        if (ammoNumber == 0)
        {
            bullet = Resources.Load("rocketBullet") as GameObject;
        }
        ...
        {
            bullet = Resources.Load("cannonBullet") as GameObject;
        }
        */
	...
	void FixedUpdate () {
        ...
        if(LS_LocalGameController.instance.RaceHasBegun)
	...
	public void fireShot() {
        ...
        GameObject shot1 = (GameObject) GameObject.Instantiate(bullet, weapon_hardpoint_1.position, actual_model.transform.rotation);
        var boundsCollider = GameObject.Find("Enviornment").transform.Find("BoundsVisual").GetComponent<Collider>();
        Physics.IgnoreCollision(boundsCollider, shot1.GetComponent<Collider>());
        Physics.IgnoreCollision(gameObject.GetComponent<Collider>(), shot1.GetComponent<Collider>());
		...
		//Otherwise, since the ray didn't hit anything, we're just going to guess and shoot the projectile in the general direction.
            ...
            shot1.GetComponent<Rigidbody>().AddForce((actual_model.transform.forward) * 9000f);
		}
